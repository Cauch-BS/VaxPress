...
class IDTComplexity:
    ...
    def _send_request(self, block: List[Dict]) -> List[Dict]:
        ...
        try:
            response = post(self._SCORE_URL, json=block, headers=headers, timeout=self._TIMEOUT)
    ...
    def scoring(self, seqs: List[str]):
        ...
        block_query = [queries[i:i + self._MAX_SEQ] for i in range(0, len(queries), self._MAX_SEQ)]
